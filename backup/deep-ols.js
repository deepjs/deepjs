	...
	.valuesEqual([3,4,5]);
		...
		query : function(q, errorIfEmpty)
		{
			...
			var func = function()
			{
				...
				if(!self._queried)
				{
					//console.log("dep.Chain.query : no queried")
					q.forEach(function (qu) {
						if(self._nodes.length > 1)
						{
							//console.log("deep.chain.query no queried and _nodes.length > 1")
							...
							//console.log("deep.chain.query no queried and _nodes.length <= 1 : ", qu, self._nodes[0])

							res = res.concat(self.querier.query(self._nodes[0].value, qu, { resultType:"full", root:self._nodes[0].root.value }));
						}
					});
				}
				else
				...
				//console.log("deep.Chain.query : results : ", q, res);
				return [deep.chain.values(self), null];
			};
		...
		extendsBackgrounds:function (entry)
		{
			...
			if(value.backgrounds !== null && typeof value.backgrounds === "object")
			{
				...
				deep.when(deep.getAll(value.backgrounds, { root:(entry.root)?entry.root.value:null })).then(function extendedsLoaded(extendeds){
					var recursion = [];

...
define(["require", "../deep"], function(require, deep) {
	...
	Unit.prototype = {
		...
		run: function(tests, options) {
			...
			var results = [];
			var d = deep.contextualise({ protocols:{}, session:null }).done(function(){ return self.setup(options); });
			var closure = {};
	...
	function reportToString(){
		...
		if (e)
			output += "error while executings tests bunch : ";
			output +=  e;
		output += "*******************************************************************";
		output += "********** tests bunch arrived to end : final report : ************";
		//console.log("\n",report,"\n");
		if(report.errors)
		{
			output += "\n\tErrors: ";
			report.errors.forEach(function(e) {
				...
				if (e.error && e.error.report)
					output += "report : " + report.toString();

			});
		}
		else 
			output += "\n\tNo errors.\n";
		output += "\tNumber of units : " + report.numberOfUnits;
		output += "\tNumber of tests : " + report.numberOfTests;
		output += "\tsuccess : " + report.success + "/" + report.numberOfTests;
		output += "\tfailure : " + report.failure + "/" + report.numberOfTests;
		output += "\tommited : " + report.ommited + "/" + report.numberOfTests;
		output += "\tommiteds : " + report.ommiteds;

		output += "\ttime : " + report.time;
		output += "\ttotalTime : " + report.totalTime;
		output += "\tcounters : " + JSON.stringify(deep.counter, null, ' ');

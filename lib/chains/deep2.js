...
function(require, utils, errors, prom, compose, protoc, querier, nodes) {
	...
	chains.Chain2 = compose.Classes(prom.Promise,
		function (state, options) {
			...
			_init:function(obj, schema){
	            ...
	            if (obj && obj._deep_query_node_)
	            {
	                ...
	                obj = deep.nodes.root(obj, schema);
			...
			var func = function(s,e) {
				var applyCallBackOrTreatment = function(callBack) {
					...
					else if(s._deep_query_node_)
					{
						s = s.value;
						if(!s)
							return prom.all(callback(s));
					}
					if(s.forEach)
						s.forEach(function(v){
							if(v._deep_query_node_)
								v = v.value;
							r.push(callback(v));
						});
					else
		...
		transform: function(transformer) {
			...
			var func = function(s, e) {
				var applyTransformer = function(transformer) {
					...
					{
						s.value = transformer(s.value)
						if(s.ancestor)
							s.ancestor.value[s.key] = s.value;
						return s.value;
					}
					else if(s._deep_array_)
						return nodes.transformNodes(s, transformer);
					else if(s.forEach)
					{
						var r = [];
		...
		run: function(funcRef, args) {
			...
			var doRun = function(node) {
				...
				if (!funcRef) {
					if(node._deep_query_node_)
					{
						if (typeof node.value !== "function")
							return;
						if (node.ancestor)
							return node.ancestor[node.key].apply(node.ancestor ,args);
						else
				...
				{
					if(node._deep_query_node_)
						return node.value[funcRef].apply(node.value, args);
					else
						return node[funcRef].apply(node, args);
				}
				else
				...
				.done(function(objects) {
					if(s._deep_query_node_)
						objects.forEach(function(object) {
							utils.up(object, s.value, s.ancestor.value, s.key);
						});
					else if(s._deep_array_)
						s.forEach(function(result) {
							objects.forEach(function(object) {
								result.value = utils.up(object, result.value, result.ancestor.value, result.key);
							});
						});
					else
						objects.forEach(function(object) {
		...
		bottom: function() {
			...
			var func = function(s, e) {
				...
				.done(function(objects) {
					if(s._deep_query_node_)
						objects.forEach(function(object) {
							utils.bottom(object, s.value, s.ancestor.value, s.key);
						});
					else if(s._deep_array_)
						s.forEach(function(result) {
							objects.forEach(function(object) {
								result.value = utils.bottom(object, result.value, result.ancestor.value, result.key);
							});
						});
					else
						objects.forEach(function(object) {
							s = utils.bottom(object, s);
						});
					return s;
				});
			};
			...
			var func = function(s, e) {
				var applySet = function(obj) {
					if(!s)
						return s;
					if(s._deep_query_node_)
						utils.fromPath(s.value, path, obj, '/');
					else if(s._deep_array_)
						s.forEach(function(v){
							utils.fromPath(v.value, path, obj, '/');
						});
					else
						utils.fromPath(s, path, obj, '/');
					return s;
				};

...
define(["require", "../stores/store", "../utils", "../errors", "../promise", "../nodes", "../compose", "./deep"],
	function(require, Store, utils, errors, prom, nodes, compose, chains) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (method._deep_ocm_)
							method = method();
						return prom.when(method.call(store, object || chains.chain.val(self), options))
							.done(function(success) {
								self._state.nodes = [nodes.root(success)];
							});
					};
					if (self._storeDef && !self._store)
						return prom.when(Store.prepare(self._storeDef)).done(doIt);
		...
		var proto = {
			range: function(arg1, arg2, query, options) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (method._deep_ocm_)
							method = method();
						return prom.when(method.call(store, arg1, arg2, query, options))
							.done(function(success) {
			...
			get: function(id, options) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (id[0] == "*")
							id = id.substring(1);
						return prom.when(method.call(store, id, options))
							.done(function(success) {
			...
			del: function(id, options) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (method._deep_ocm_)
							method = method();
						return prom.when(method.call(store, id || val.id, options))
							.done(function(success) {
								self._state.nodes = [nodes.root(success)];
							});
					};
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (action._deep_ocm_)
							action = action();
						return prom.when(action.call(store, method, args, uri, options))
							.done(function(success) {
								self._state.nodes = [nodes.root(success)];
							});
					};
			...
			flush: function(method, body, uri, options) {
				var self = this;
				var func = function(s, e) {
					var doIt = function(store) {
						self._store = store;
			...
			bulk: function(arr, uri, options) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (method._deep_ocm_)
							method = method();
						return prom.when(method.call(store, arr, uri, options))
							.done(function(success) {
								self._state.nodes = [nodes.root(success)];
							});
					};
			...
			count: function(callback) {
				...
				var func = function(s, e) {
					var doIt = function(store) {
						...
						if (method._deep_ocm_)
							method = method();
						return prom.when(method.call(store))
							.done(callback);
					};
					if (self._storeDef && !self._store)
						return prom.when(Store.prepare(self._storeDef)).done(doIt);
		...
		var constructor = function(state, store) {
			...
			if (store._deep_request_)
				this._storeDef = store.handler.provider;
			...
			var self = this;
			var func = function(s, e) {
				return prom.when(Store.prepare(self._storeDef))
					.done(function(st) {
						self._store = st;
					});
			};
		...
		var Chain = compose.Classes(prom.Promise, constructor, proto);
		Chain._aspects = {
			constructor: constructor,
			proto: proto
		};
		Chain.start = function(store) {
			return new Chain(null, store)._start();
		};
		Chain.start.Chain = Chain;
		prom.Promise.API.rest = function(store) {

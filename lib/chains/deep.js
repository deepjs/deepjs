...
function(require, utils, errors, prom, compose, protoc, querier, nodes, compiler) {
	...
	var DeepChain = compose.Classes(prom.Promise,
				...
				.done(function(objects) {
					if(s._deep_query_node_)
						objects.forEach(function(object) {
							compiler.up(object, s.value, s.ancestor.value, s.key);
						});
					else if(s._deep_array_)
						s.forEach(function(result) {
							objects.forEach(function(object) {
								result.value = compiler.up(object, result.value, result.ancestor.value, result.key);
							});
						});
					else
						objects.forEach(function(object) {
			...
			var func = function(s, e) {
				...
				if(s._deep_array_)
				{
					...
					s.forEach(function(n){
						r = r.concat(deep.query(s, q, { resultType:"full" }));
					});
					if(r.length)
						utils.arrayUnique(r, "path");
					r._deep_array_ = true;
					return r;
				}
				var r = deep.query(s, q, { resultType:"full" });
	...
	.mapOn([
		...
		mapOn: function(what, localKey, foreignKey, whereToStore) {
			...
			var func = function(s, e) {
				...
				if (typeof what === 'string') {
					...
					var foreigns;
					if(s._deep_array_)
						foreigns = deep.query(s, "./*/value/" + localKey).join(",");
					else if(s.forEach)
						foreigns = deep.query(s, "./*/" + localKey).join(",");
					else
						foreigns = deep.query(s, "./" + localKey);
		...
		val:function(callback){
			...
			var func = function(s, e) {
				if(!callback)
					return deep.nodes.val(s); 
				return callback(deep.nodes.val(s));
			};
			func._isDone_ = true;
			return self._enqueue(func);
		}

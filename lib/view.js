	...
	deep.ui.View.htmlEnhancers = {
		...
		"dp-control":function(node, route){
			...
			.done(function(ctrl) {
				...
				controller.subs = controller.subs || {};
				var label = ctrl.label;
				if(!label || controller.subs[label])
					label = "view"+Date.now().valueOf();
				controller.subs[label] = ctrl;
				deep.utils.up({ clean:deep.compose.after(function(){
					delete controller.subs[label];
				}) }, ctrl);
				ctrl.where = $(node);
				return ctrl.refresh();
			});

...
define(["require", "../deep"], function(require, deep) {
			...
			var render = function(loaded) {
				...
				if (placed)
				{
					...
					$(placed).find("*").each(function() {
						...
						console.log("inside filter = ", this.attributes);

						// for (var property in $(this).data()) {
						// 	console.log("Inside thisdata : property = ", property);
						// 	if (property.indexOf('dp-') === 0) {
						// 		console.log("Node got a dp-args : ", property);
						// 		return true;
						// 	}
						// }
						// return false;
	...
	deep.ui.View.htmlEnhancers = {
		...
		"dp-click":function(success){
			//console.log("DPCLICK : descriptor = ", success);
			var promises = [];
			success.placed
			.find("*[dp-click]")
			.each(function() {
				//console.log("inside a dpclick tag :", $(this));
				var node = $(this);
				var action = node.attr("dp-click");
				var args = node.attr("dpargs");

				node.click(success.controller[action]);
				console.log("inside a dpclick tag : Action = ", action, " Args : ", args);

			});
			return true;
		},
		"dp-repeat":function(success){

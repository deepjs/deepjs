...
*/
	...
	deep.ui.View = deep.compose.Classes(function(layer) {
		...
		refresh: function(arg /* params, loaded, routeMatch */) {
			...
			var self = this, loaded, params, routeMatch;
			if(arg)
			...
			var render = function(loaded) {
				...
				try {
					...
					else if(typeof jQuery !== 'undefined' && self.where instanceof jQuery)
						placed = self.where;
					else
		...
		enhance: function(selector) {
			...
			{
				if(deep.route && deep.route.relink)
					deep.route.relink(selector);
				return null;
			}
			return deep.all(promises)
			.done(function(){
				if(deep.route && deep.route.relinkNode)
					deep.route.relink(selector);
			})
		},
	...
	deep.ui.enhance = function(selector, ctrl){
		return deep.ui.View.prototype.enhance.call(ctrl || {}, selector);
	}

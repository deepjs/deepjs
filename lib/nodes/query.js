...
define(["require", "./nodes", "../errors", "./rql", "../utils/array", "../utils/schema", "../utils/string"], function (require, nodes, errors, rqler, arrayUtils, schemaUtils, stringUtils) {
	"use strict";
	/*
	deep.coreUnits = deep.coreUnits || [];
	deep.coreUnits.push("req::deep/units/queries");
*/
	var QueryError = function(msg, report, fileName, lineNum) {
		...
		if (!msg)
			msg = "Login";
		return errors.Error(400, msg, report, fileName, lineNum);
	};
	var retrieveFullSchemaByPath = schemaUtils.retrieveFullSchemaByPath;
	...
	DQ.prototype.analyseEreg = function analyseEreg(path, parts) {
		...
		var parsed = stringUtils.catchParenthesis(path);
	...
	DQ.prototype.analyseSelector = function dqanalyseSelector(path, parts, fromUnion) {
		...
		if (path[0] == "?") {
			...
			if (fromUnion)
				throw new QueryError("you couldn't have '?' in union of selectors.", path, parts);

...
define(["require", "../promise", "../utils", "../compiler", "../utils/logs", "../protocol"], function(require, prom, utils, compiler, logs, proto) {

	var nodes = {
		val: function(s) {	// return value(s) from node(s)
			if (!s)
				return s;
			if (s._deep_array_)
		...
		paths: function(nodes) {	// return paths from node(s)
		...
		schemas: function(nodes) {	// return schemas from node(s)
			if (nodes._deep_query_node_)
				return [nodes.schema];
			return nodes.map(function(e) {
				return e.schema;
		...
		each: function(node, transformer) {  // loop on node(s)
		...
		map: function(node, transformer) {  // transform node(s) value(s)
			if(!node)
				return transformer(node);
			if (node._deep_query_node_) {
		...
		asyncUps:function(s, objects){	// load 

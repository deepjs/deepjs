...
define(["require","../../deep"],function (require, deep)
{
	...
	var retrieveSchema = function (object,options) {
		...
		if(typeof this.schema == 'string')
		{
			...
			this.schema = deep.get(this.schema)
			.done(function (schema) {
				//console.log("schema is resolved")
				self.schema = schema;
			})
			.logError();
		...
		else if(this.schema._deep_promise_)
			return deep.when(this.schema)
			.done(function () {
	...
	var serveSchema = function (old) {
		return function (id,options) {
			...
			if(typeof schema === 'string')
			{
				this.schema = deep.get(this.schema).done(function (schema) {
					self.schema = schema;
				});
				return this.schema;
			}
	...
	var clientStoreProto = {
		...
		post:deep.compose.around(withBody).before(schemaValidation("post")).before(retrieveSchema),
		put:deep.compose.around(withBody).before(schemaValidation("put")).before(retrieveSchema),
		patch:deep.compose.around(withBody).before(schemaValidation("patch")).before(retrieveSchema),

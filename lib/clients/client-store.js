...
define(["require", "../../deep"],function (require, deep)
{
	...
	var noBody = function(old){
		return function(id, options){
			...
			if(deep.client.decorateHeaders)
				deep.client.decorateHeaders(options.headers);
	...
	var withBody = function(old){
		return function(object, options){
			...
			if(!options)
				...
				options = { id:options||'' };
			options.id = (deep.context.rootPath || deep.globals.rootPath || "")+this.baseURI+options.id;
			options.headers = deep.utils.compile(this.headers, deep.globalHeaders, options.headers || {});
			if(deep.client.decorateHeaders)
				deep.client.decorateHeaders(options.headers);
			if(this.decorateHeaders)
	...
	var clientStoreProto = {
		...
		get:deep.compose.around(noBody).around(serveSchema)/*.around(function(old){
			return function(id, options){

 ...
 * @submodule deep-rql
 * @example
 * deep.rql([{a:{b:3}},{a:3}], "a.b=3") -> [{a:{b:3}]
 * @author Gilles Coomans <gilles.coomans@gmail.com>
 */
if (typeof define !== 'function') {
    var define = require('amdefine')(module);
}
define(["require","../deep", "rql/parser"], function (require){
return function(deep)
{
	var rqlParser = require("rql/parser").parseQuery;
	deep.utils.parseRQL = function(input){
		try {
	...
	deep.rql.ops = {
		...
		contains: filter(function rqlContains(array, value){
			return deep.utils.inArray(value, array);
		}),
		...
		or: function rqlOr(){
			...
			for(var i = 0; i< arguments.length; ++i)
			{
				...
				if(typeof a == 'function')
					...
					items = items.concat(deep.rql.ops.isPresent(a, items));
			}
			return items;
		},
		and: function rqlAnd(){
			...
			for(var i = 0; i< arguments.length; ++i){
				...
				if(typeof a == 'function')
					...
					items = deep.rql.ops.isPresent(a, items);
			}
			return items;
		},
		...
		mean: function rqlMean(property){
			return deep.rql.ops.sum.call(this, property)/this.length;
		},
	...
	function rqlNodeToFunc(node)
	{
		if(typeof node === 'object')
		{
			...
			else
			{
				...
				switch(name)
				{
					case "eq" : isFilter = true; func = function eq(a){ return (retrieve(a, path) || undefined)===b; }; break;
					case "ne" : isFilter = true; func = function ne(a){ return (retrieve(a, path) || undefined)!==b; }; break;
					case "le" : isFilter = true; func = function le(a){ return (retrieve(a, path) || undefined)<=b; }; break;
					case "ge" : isFilter = true; func = function ge(a){ return (retrieve(a, path) || undefined)>=b; }; break;
					case "lt" : isFilter = true; func = function lt(a){ return (retrieve(a, path) || undefined)<b; }; break;
					case "gt" : isFilter = true; func = function gt(a){ return (retrieve(a, path) || undefined)>b; }; break;
					default :
						var ops = deep.rql.ops[name];
						if(!ops)
							throw deep.errors.RQL("no operator found in rql with : "+name);
				...
				if(isFilter)
					return function(){  var r = this.filter(func); return r; };
				else
					return func;
			}
		}
	...
	function retrieve(obj, path)
	{
		...
		switch(splitted[0])
		{
			...
			case '_type' :
				if(obj._deep_query_node_)
					return deep.utils.getJSPrimitiveType(obj.value);
				return deep.utils.getJSPrimitiveType(obj);
			default :
				if(obj._deep_query_node_)
					tmp = tmp.value;
		}
		if(!tmp)
			return tmp;
		var len = splitted.length, needtype = false;
		...
		if(len < 5)
		{
			...
			{
				case 1:
					res = tmp[splitted[0]];  break;
				case 2 :
					res = tmp[splitted[0]] && tmp[splitted[0]][splitted[1]]; break;
				case 3 :
					res = tmp[splitted[0]] && tmp[splitted[0]][splitted[1]] && tmp[splitted[0]][splitted[1]][splitted[2]];  break;
				case 4 :
					res = tmp[splitted[0]] && tmp[splitted[0]][splitted[1]] && tmp[splitted[0]][splitted[1]][splitted[2]] && tmp[splitted[0]][splitted[1]][splitted[2]][splitted[3]];  break;
			}
			if(typeof res !== 'undefined' && needtype)
				return deep.utils.getJSPrimitiveType(res);
			return res;
		}
		...
		{
			if(needtype)
				return deep.utils.getJSPrimitiveType(tmp);
			return tmp;
		}
		return undefined;
	}

	...
	utils.decorateUpFrom = function(src, target, properties) {
		properties.forEach(function(prop) {
			if (typeof src[prop] !== 'undefined')
				target[prop] = compiler.up(src[prop], target[prop]);
		});
	};
	utils.decorateBottomFrom = function(src, target, properties) {
		properties.forEach(function(prop) {
			if (typeof src[prop] !== 'undefined')
				target[prop] = compiler.bottom(src[prop], target[prop]);
		});
	};
...
*/
	...
	compiler.aup = function(src, target, opt){
		...
		if (typeof target === 'undefined' || target === null) {
			...
			if(opt && opt.excludeGrounds)
			{
				delete target._foregrounds;
				delete target._backgrounds;
			}
			return target;
		}
		/*if (src._deep_sheet_) {
		...
		switch (srcType) {
			...
			case 'object':
				...
				for (var i in src) {
					if(opt && opt.excludeGrounds && (i == '_backgrounds' || i == '_foregrounds'))
						continue;
	...
	utils.up = compiler.up = function() {
		...
		for (var i = arguments.length - 2; i > 0; i--) {
			...
			target = compiler.aup(argi, target);
		}
		return compiler.aup(src, target);
	};
	...
	utils.bottom = compiler.bottom = function() {
		...
		for (var i = arguments.length - 2; i > 0; i--) {
			var argi = arguments[i];
			target = compiler.abottom(argi, target);
		}

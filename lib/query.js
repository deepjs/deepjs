...
*/
	var QueryError = function  (msg, report, fileName, lineNum) {
		...
		if(!msg)
			msg = "Login";
		return deep.errors.Error(400, msg, report, fileName, lineNum);
	};
 ...
 */
		...
		if(path[0] == '(')
		{
			...
			parts.push({
				...
				handler:function(items){
					...
					return deep.rql(items, parsed.value);
				}
			});
			return parsed.rest;
		}
	...
	DQ.prototype.query = function doDeepQuery(obj, q, options)
	{
		...
		options = options || {};
		var items = null, currentRoot = null;
		...
		else
		{
			...
			currentRoot = deep.nodes.root(obj, options.schema);
		...
		var self = this, start = true, count = 1, part = parts[0];
		while(part)
		{
			switch(part.type)
			{

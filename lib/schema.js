...
define(["require", "deepjs/deep", "deepjs/lib/utils"], function(require, deep, utils) {
...
*/
		...
		if (schema.dependencies && schema.dependencies.length > 0) {
			...
			schema.dependencies.forEach(function(dep) {
				...
				if (res.length > 0) {
					...
					var finSchema = utils.up(schema, {});
					utils.up(dep.constraints, finSchema);
						...
						if (res.length > 0) {
							...
							for (var i in schema) {
								if (!schema.hasOwnProperty(i) || i == "dependencies")
									continue;
								schemaCopied[i] = utils.up(schema[i], {});
							}
							schemaCopied = utils.up(dep.constraints, schemaCopied);
	...
	utils.up(draftv4, Validator.prototype.lexic);
	utils.up(jsonExt, Validator.prototype.lexic);
	utils.up(newSpec, Validator.prototype.lexic);
	//utils.up(se_spec, Validator.prototype.lexic);

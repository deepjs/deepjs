...
*/
		...
		if (schema.dependencies && schema.dependencies.length > 0) {
			...
			schema.dependencies.forEach(function(dep) {
				...
				if (res.length > 0) {
					...
					var finSchema = deep.up(schema, {});
					deep.up(dep.constraints, finSchema);
						...
						if (res.length > 0) {
							...
							for (var i in schema) {
								if (!schema.hasOwnProperty(i) || i == "dependencies")
									continue;
								schemaCopied[i] = deep.up(schema[i], {});
							}
							schemaCopied = deep.up(dep.constraints, schemaCopied);
	...
	deep.up(draftv4, Validator.prototype.lexic);
	deep.up(jsonExt, Validator.prototype.lexic);
	deep.up(newSpec, Validator.prototype.lexic);
	//deep.up(se_spec, Validator.prototype.lexic);

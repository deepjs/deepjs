...
function(require, utils, errors, prom, compose, protoc, querier, nodes) {
	...
	var chains = {};
	chains.api = {};
	//chains.ocmapi = deep.ocm(chains.api);
	...
	chains.Chain = compose.Classes(prom.Promise,
		function (options) {
			...
			this._queried = options._queried || false;
			this._root = options._root || null;
			this._nodes = options._nodes || null; // || [nodes.root(this._value, options.schema, options)];
			...
			_forward:compose.around(function(old){
				return function(clone){
					...
					cloned._nodes = (clone?this._nodes.slice():this._nodes);
					cloned._queried = this._queried;
			...
			/*promise: function() {
				...
				if (this._initialised && this._state.queue.length === 0 && (!this._state.oldQueue || this._state.oldQueue.length === 0) && !this._running && !this._executing)
					if (!this._state.error)
						this.deferred.resolve(this._state.success);
					else
						this.deferred.reject(this._state.error);
				return this.deferred.promise();
			},
			_next: function chainForce() {
				var self = this;
				...
				if (!self.deferred)
					return;
				if (self._state.queue.length === 0 && !self._running)
					if (self._state.error)
						self.deferred.reject(self._state.error);
					else
						self.deferred.resolve(self._state.success);
			},*/
			/*clone: function(cloneValues) {
				throw Error("try clone chain : "+ cloneValues)
				...
				var newHandler = new chains.Chain({
					...
					_error: handler._state.error,
					_context: handler._context,
					_success: handler._state.success,

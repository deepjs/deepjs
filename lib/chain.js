...
define(["require"], function (require) {
	...
	deep.Chain = deep.compose.Classes(deep.BaseChain,
	{
				...
				self._nodes.forEach(function (node) {
					...
					else if(typeof value === 'object')
					{
						if(!destructive)
						{
							...
							node.value = deep.utils.copy(value);
						}
						var toLoads = deep.query(node, ".//!?or(_type=string,_type=function)", {
							resultType: "full"
						});
						var p = deep.chain.transformNodes(toLoads, doDeepLoad)
						.done(function(e){
							return node.value;
						});
						promises.push(p);
		 ...
		 * @return {deep.Chain} this
		 */
		load: function chainLoad(context, destructive) {
			var self = this;
			if(typeof destructive === 'undefined')
				destructive = false;
			var func = function (s, e) {
				...
				var doLoad = function (v) {
					...
					if (!v.value)
						return v.value;
					if (v.value.load)
						return deep.when(callFunctionFromValue(v, "load", [context]));
					else if (typeof v.value === 'string')
					{
						var val = v.value;
						if(context)
							val = deep.utils.interpret(val, context);
						return deep.when(deep.get(val, {entry:v}))
						.done(function(r){
	...
	function callFunctionFromValue(entry, functionName, args) {
		if (!entry._deep_query_node_)
			throw deep.errors.Error(500, "deep.callFunctionFromValue need DQNode");
	...
	deep.chain = {
		...
		first: function utilsFirst(handler, modifyNodes) {
			console.log("deep.chain.first : ", handler, modifyNodes)
			if(!handler._nodes)
				return deep.Undefined;
			if (handler._nodes && handler._nodes.length === 0)
				return deep.Undefined;

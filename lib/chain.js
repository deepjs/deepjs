...
define(["require"], function (require) {
	...
	var addInChain = deep.utils.addInChain;
	var forceHandle = deep.utils.forceHandle;
	var utils = deep.utils;

	deep.BaseChain = deep.compose.Classes(deep.Promise,
	function BaseChainConstructor(options) {
		options = options || {};
		delete this._deep_promise_;
		this._deep_chain_ = true;
		this._queried = options._queried;
		this._nodes = options._nodes; // || [deep.utils.createRootNode(this._value, options.schema, options)];
		this.positions = [];
	...
	deep.Chain = deep.compose.Classes(deep.BaseChain,
	{
		/**
		 *
			...
			var func = function (s, e) {
				var doDeepLoad = function (toLoad) {
					if (typeof toLoad.value === 'string') {
						...
						var d = null;
						if (toLoad.ancestor)
							d = toLoad.ancestor.value[toLoad.key]();
						else
							d = toLoad.value();
						return deep.when(d);
					} else
				...
				self._nodes.forEach(function (node) {
					...
					if(typeof value === "string")
					{
						...
						promises.push(deep.get(value, { entry: node }));
					}
					...
					{
						var oldValue = value, newValue = value;
						if(!destructive)
						{
							node = deep.utils.simpleCopy(node);
							node.value = newValue = deep.utils.copy(value);
							if(node.ancestor)
								node.ancestor.value[node.key] = newValue;
							//console.log("deep.utils.simpleCopy(node) : ", deep.node)
						}
						var toLoads = deep.query(node, ".//*?or(_schema.type=string,_schema.type=function)", {
							resultType: "full"
						});
	
						var p = deep.chain.transformNodes(toLoads, doDeepLoad).done(function(e){
							//console.log("deep load transfo gives : ",e, " - node.value : ", node.value, " - value : ", value, "\n\n\noldValue", oldValue);
							return newValue;
						});
						if(!destructive)
						{
							if(node.ancestor)
								node.ancestor.value[node.key] = oldValue;
						}
						promises.push(p);
					}
				});
				...
				.done(function(res){
					//console.log("Rezz deepload :", res, self._queried);
					if(!self._queried)
						return res.shift();
					return res;
				});
		...
		mapOn: function chainMapOn(what, localKey, foreignKey, whereToStore) {
			...
			var func = function (s, e) {
				...
				if (typeof what === 'string') {
					var parsed = deep.utils.parseRequest(what);
			...
			var func = function (s, e) {
				...
				self._nodes.forEach(function (entry) {
					...
					.forEach(function (relation) {
						...
						var parsed = deep.utils.parseRequest(path);
						var wrap = {
							rel: relation
							//href: parsed
						};
	...
	deep.Chain.addHandle("logState", function () {
		...
		var func = function (s, e) {
			if (e)
				console.log("deep.Chain state : ERROR : ", deep.utils.dumpError(e));//JSON.stringify(e, null, ' '));
			else
	...
	deep.Chain.addHandle("iterate", function (done, fail) {
		...
		var func = function (s, e) {
			...
			return deep.iterate(deep.chain.values(self), done, fail);
		};
		func._isDone_ = true;
		addInChain.apply(self, [func]);
		return this;

...
define(["require", "./utils", "./compiler", "./nodes"], function (require, utils, compiler, nodes)
{
	...
	flattener.extendsBackgrounds = function (entry)
	{
		...
		var finalise = function(extendeds){
			...
			//if(bck._deep_sheet_ === entry.value._deep_sheet_)	// false or true
			//{
			entry.value = utils.bottom(bck, entry.value);
			if(entry.ancestor)
				entry.ancestor.value[entry.key] = entry.value;
			/*}
			else if(entry.value._deep_sheet_)
			{
				sheets.push(deep.sheet(entry.value, bck));
				entry.value = bck;
				if(entry.ancestor)
					entry.ancestor.value[entry.key] = entry.value;
			}*/
			...
			// for(;len>=0;len--)
			// {
			// 	var val = extendeds[len];
			// 	entry.value = utils.bottom(val, entry.value);
			// 	if(entry.ancestor)
			// 		entry.ancestor.value[entry.key] = entry.value;
			// }

...
flattener.developStack = function(loadeds, entry) {
	// console.log("developStack : ", loadeds);
	var stack = [];
	var needLoad = false, len = loadeds.length;
	for (var i = 0; i < len; ++i) {
		var s = loadeds[i], r;
		if (s && s._backgrounds) {
...
var recurse = function(array, entry) {
	...
	if (!needLoad)
		return flattener.developStack(all, entry);
	return deep.all(all)
	.done(function(res){
		return flattener.developStack(res, entry);
	});
};
...
var flattenEntry = function(entry){
	var promises = [];
	var obj = entry.value, r, wait, d;
	if(obj._backgrounds)
	{
		r = flattener.developStack(obj._backgrounds, entry);
		promises.push(r);
		if(r && r.then)
			wait = true;
	}
	if(obj._foregrounds)
	{
		r = flattener.developStack(obj._foregrounds, entry);
		promises.push(r);
		if(r && r.then)
			wait = true;
	}
...
deep.when(deep.flattener.developStack([a]))
.done(function(s) {
	return deep.utils.compile.apply(deep, s);
})
.equal({
	"bloup": true,
	"backback": true,

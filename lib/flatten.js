...
define(["require", "./utils", "./compiler", "./nodes", "./traversal"], function(require, utils, compiler, nodes, traversal) {
	...
	flattener.extendsChilds = function(entry, descriptor) {
		...
		descriptor = traversal.depthFirst(descriptor || entry, function(value) {
			return value._backgrounds || value._deep_flattener_;
		}, {
			first: true,
			returnStack: true,
			excludeLeafs: true
		...
		if (toExtends.value._deep_flattener_)
			return toExtends.value.flatten(toExtends).done(function() {
				return entry;
			});
	...
	flattener.extendsBackgrounds = function(entry) {
		...
		if (!value._backgrounds) {
			if (value._sheets)
				return deep.getAll(value._sheets, {
					entry: entry
				})
				.done(function(success) {
					delete value._sheets;
					success = deep.utils.compile.apply({}, success);
					return deep.sheet(success, value);
				})
				.done(function() {
					return entry;
				});

...
define(["require", "./nodes", "./traversal", "./promise", "./compiler", "./sheet", "./protocol"], function(require, nodes, traversal, prom, compiler, sheets, proto) {
	var flattener = {};
		...
		while (stacki) {
			...
			if (tmp.length > 0) {
				chain.done(function(s) {
					tmp.push(s);
					return compiler.up.apply(deep, tmp);
				});
			}
			if (!stacki)
				break;
			var sheet = {};
			while (stacki && stacki._deep_sheet_) {
				sheet = compiler.up(stacki, sheet);
				stacki = stack[++i];
			}
			chain.done(function(s) {
				if (!s)
					return sheet;
				if (!s._deep_sheet_)
					return sheets.sheet(sheet, s);
				else
					return sheets.up(sheet, s);
			});
		}
	}
	...
	var flattenEntry = function(entry) {
		...
		if (wait)
			...
			d = prom.when.immediate(promises);
		return d.done(function(res) {
			...
			if (obj._backgrounds) {
				...
				this.done(function(bck) {
					if (bck)
						return compiler.bottom(bck, obj);
					return obj;
				});
			}
			if (obj._foregrounds) {

...
define(["require", "./compiler", "./errors", "./utils"], function(require, compiler, errors, utils){
	...
	var compose = {
		...
		Classes : function(){
			...
			for(var i = 0; i < args.length; ++i)
			{
				...
				{
					//console.warn("You try to compose with an undefined class");
					throw errors.Composition("You try to compose with an undefined class");
				}
				if(cl._deep_ocm_)
					args[i] = cl = cl();
				if(typeof cl === 'function')
				{
					if(cl.prototype)
						prototype = utils.up(cl.prototype, prototype);
				}
				else
					prototype = utils.up(cl, prototype);
			}
		...
		ClassFactory : function(){
			...
			var Factory = function (modes){
				...
				.modes(mds)
				.done(function(){
					return compose.Classes.apply(compose, args);
				})
				.done();
			};
			return Factory;
	...
	var Composer = compose.Composer = function (fn)
	{
		...
		comp.clone = function (){
			var c = Composer(compClosure.queue);
			return c;
		};
		...
		for(var i in compose.compositions)
			comp[i] = compose.compositions[i];
		return comp;
	};
	...
	Composer.add = function(name, type, method){
		var h = function(){
			var doIt = method.apply({}, arguments);
			doIt._deep_composition_ = type;
			if(!this._deep_composer_)
				return Composer(doIt);
			this.compiled = null;
			this.queue(doIt);
			return this;
		};
		compose.compositions[name] = compose[name] = h;
		return Composer;
	};

	var compile = Composer.compile = function (compClosure)
	{
	...
	Composer.add("log", "after", function(){
			return function(){ 
...
/*
	Composer.add("myAround", "around", function(onlyTag, normalize){
		return function(old){
			return function(){ 
	...
	Composer.add("sanitize", "before", function(onlyTag, normalize){
		return function(){

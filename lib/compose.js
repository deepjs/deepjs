 ...
 *
 *
 * TODO : add deep.Arguments([]) or deep.Arguments(1,2,3)
 */
...
define(function(require, exports, module){
	...
	deep.Composer = function ComposerConstructor(fn)
	{
		...
		{
			if(!compClosure.compiled)  // need to reduce queue to one element
	...
	deep.Composer.chain = function compoAddChain(before, after, closure)
	{
		return function compoDoChain(){
			...
			//console.log("chain finished after call : ", d._finished)
			if(!d._finished)
			{
				asynch = true;
				return d;
	...
	var compile = deep.Composer.compile = function compoDOCompil(compClosure)
	{
				...
				case "fail" :
					...
					func = function compoDOFail(){
						...
						try{
							...
							//console.log("fail finished after call : ", d._finished, d._success)
							if(!d._finished)
								return d;
							if(d._error)
								return d._error;
		...
		/*func = function(){
			...
			if(r && r._deep_arguments_)
				delete r._deep_arguments_;
			return r; 
		};*/
		return func;
	};

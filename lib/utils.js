...
define(function(require){
	...
	*/
	...
	utils.dumpError  =function(err, args)
	{
		if (typeof err === 'object' && err !== null)
		{
			...
			if(args)
			{
				if(args[0] === "dp:error")
					args.shift();
				deep.log((args.join)?args.join(" - "):args);
			}
			if (err.status)
				deep.log('\nStatus: ' + err.status);
			//if (err.message)
			...
			if(err.requireModules)
				deep.log("Error from : ", err.requireModules);
			if (!console.error && err.stack)
			{
				deep.log('\nStacktrace:');
				deep.log('====================');
				deep.log(err.stack);
			...
			if (err.report)
			{
				deep.log('\nReport:');
				deep.log('====================');
				deep.log(JSON.stringify(err.report, null, ' '));
				deep.log('============================================================');
			}
			deep.log("\n")
		}
		else
			deep.warn('dumpError :: argument is not an object : ',err);
	};
	...
	utils.remove = function(obj, what, schema){
		...
		r = deep.query(obj, what, { fullOutput:true, schema:schema });
	...
	utils.replace = function(target, what, by, schema){
		...
		var r = deep.query(target, what, { fullOutput:true, schema:schema });
		if (!r)
			return r;
		if (r._deep_query_node_)
		{

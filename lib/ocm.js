...
define(["require", "./utils/misc", "./promise", "./flatten", "./sheet", "./errors", "./protocol", "./compiler", "./utils/logs"],
	function(require, utils, prom, flattener, sheet, errors, protoc, compiler, logger) {
     ...
     */
			...
			var params = {
				...
				compile: function(modes, layer) {
					...
					if (this.multiModes === false && modes.length > 1) {
						logger.warn("OCM : no mulitMode allowed : provided modes : ", modes, " (protocol : ", options.protocol, ")");
						return res;
					}
					var ok = modes.every(function(m) {
						...
						if (typeof r === 'undefined') {
							if (self.strict) {
								logger.warn("OCM : no associate entry found with : ", modes, " (protocol : ", options.protocol, ")");
								return false;
							}
							return true;
						}
        ...
        m.unblock = function(key) {};*/
			m.flatten = function(entry, force) { // flatten inner-layer
				...
				// if(entry)
					// console.log("flatten inner layer ocm on entry : ", entry.path, entry.value);
				if (params.blocked)
				...
				var def = params.flattened = new prom.Promise();
				prom.when(flattener
		...
		ocm.matchModes = function (mode, value){
			return utils.inArray(value, ocm.currentModes(mode));
		};

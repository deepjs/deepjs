			...
			var m = function() {
				...
				if (modes.length === 0 /* || params.blocked*/ )
				{
					if (params.currentModes && params.currentModes.length > 0)
						...
						} else if (prom.Promise.context.modes && prom.Promise.context.modes[params.groups])
							modes = modes.concat(prom.Promise.context.modes[params.groups]);

...
return function(deep)
{
    ...
    deep.protocol.getStoreHandler = function(protocol, opt)
    {
        ...
        else if(deep.context.protocols && deep.context.protocols[protocol])
            handler.store = deep.context.protocols[protocol];
    ...
    deep.utils.parseRequest = function (request, options) {
        ...
        if(protoc)
        {
            var argsPresence = protoc.indexOf("(");
            if(argsPresence > -1)
            {
                var parenthesisRes = deep.utils.catchParenthesis(protoc.substring(argsPresence));
                protoc = protoc.substring(0,argsPresence);
                if(parenthesisRes)
                {
                    var rangeSplit = parenthesisRes.value.split(",");
                    range = { start:parseInt(rangeSplit[0]), end:parseInt(rangeSplit[1]) };
                }
            }
        }
    ...
    deep.get = function (request, options) {
        ...
        .done(function(storeHandler){
            ...
            if(infos.range)
            {
                if(!storeHandler.store.range)
                    return deep.errors.Store("no range in store !.", infos);
                res = storeHandler.store.range(infos.range.start, infos.range.end, infos.uri, options);
            }
            else
                res = storeHandler.store[storeHandler.method](infos.uri, options);
            if (options.wrap)
            {

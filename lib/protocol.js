...
define(["require", "./utils", "./errors"], function(require, utils, errors) {
	...
	var parseProtocol = function(protocol, output)
	{
		...
		output.provider = deep.protocol(protocol);
		if (!output.provider)
			output.provider = deep.errors.Protocol("no provider found with : " + protocol);
	...
	utils.parseRequest = function(request) {
		...
		return parseProtocol(protoc, response);
	};
	...
	proto.protocols = {
		...
		dq: {
			get: function dqGet(request, options) {
				...
				if (!entry)
					return undefined;
				var root = entry.root || entry, uri = request;
				if (request._deep_request_)
					uri = request.uri;
				if(uri[0] == '#')
					uri = uri.substring(1);

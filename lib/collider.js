...
define(["require", "./utils", "./compiler", "./errors"], function(require, utils, compiler, errors) {
	/**
	...
	Collider.add("bottom", function() {
		...
		for (; len > 0; --len)
			compiler.bottom(arguments[len], base);
		return base;
	});
		/**
	 * up
	...
	Collider.add("up", function() {
		...
		for (var count = 1; count < len; ++count)
			compiler.up(arguments[count], base);
		return base;
	});
		/**
	 * replace
	...
	Collider.add("removeAt", function(input, index, howMuch) {
		if (!(input instanceof Array))
			throw errors.Internal("colliders.removeAt couldn't be applied : target is not an array.");

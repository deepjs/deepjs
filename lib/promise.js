...
define(["require", "./utils", "./errors"], function PromiseDefine(require, utils, errors){
    ...
    promise.when.immediate = function (result) {
        ...
        return prom._start({success:result});
    };
    ...
    promise.all = function () {
        ...
        if(arr.length == 1)
        {
            if(arr[0] && (arr[0].then || arr[0].promise))
                return promise.when(arr[0])
                .done(function(s){
                    return [s];
                });
            return deep.when.immediate(arr);
        }
        var def = promise.Deferred();
    ...
    promise.Deferred.prototype = {
        ...
        resolve: function (argument) {
            ...
            this._promises.forEach(function (promise) {
                promise._start({success:argument, error:null});
            });
        },
        /**
        ...
        reject: function (argument) {
            ...
            var self = this;
            this._promises.forEach(function (promise) {
                promise._start({success:null, error:argument});
            });
        },
        /**
        ...
        promise: function () {
            ...
            if (this.resolved || this.rejected || this.canceled)
                return prom._start({success:this._success, error:this._error/*, this._context*/});
    ...
    var Promise = promise.Promise = function (state, options) {
        ...
        this._state.success = state.success || null;
        ...
        this._state.rethrow = (typeof state.rethrow !== "undefined") ? state.rethrow : deep.rethrow;
    ...
    promise.Promise.prototype = {
        ...
        catchError: function (arg) {
            ...
            if (self._initialised) {
                var func = function (s, e) {
                    self._state.rethrow = (typeof arg !== 'undefined') ? !arg : false;
                };
                self._enqueue(func);
            } else
                self._state.rethrow = (typeof arg !== 'undefined') ? !arg : false;
                ...
                if (args.length === 0) {
                    if (e)
                    {
                        if (deep.debug)
                            ...
                            args.push("dp:error : (" + e.status + "): ", e.message, e.report);
                        else
                            args.push("dp:error : (" + e.status + "): ", e.message);
                    }
                    else
                        args.push("dp:success : ", s);
                }
        ...
        branches: function (func) {
            var self = this;
            var create = function(s, e) {
                //self._state.oldQueue = self.callQueue;
                //self._state.queue = [];

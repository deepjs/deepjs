...
define(["require", "./utils", "./errors"], function PromiseDefine(require, utils, errors){
    ...
    promise.when.immediate = function (result) {
        ...
        return prom._start({_success:result});
    };
    ...
    var addInChain = utils.addInChain = function (handle) {
        var self = this;
        if (!this._oldQueue && self.deferred && self.deferred._promises && self.deferred._promises.length > 0)
    ...
    var forceHandle = utils.forceHandle = function forceChainHandle() {
        ...
        if (this._oldQueue) {
            this._queue = this._queue.concat(this._oldQueue);
            delete this._oldQueue;
        }
        ...
        if (self._queue.length !== 0) {
            ...
            while (!self._running) // while not asynch
            {
                ...
                try {
                    ...
                    if (self._oldQueue) {
                        self._queue = self._queue.concat(self._oldQueue);
                        delete self._oldQueue;
                    }
                }
            }
    ...
    promise.Deferred.prototype = {
        ...
        resolve: function (argument) {
            ...
            this._promises.forEach(function (promise) {
                promise._start({_success:argument, _error:null});
            });
        },
        /**
        ...
        reject: function (argument) {
            ...
            var self = this;
            this._promises.forEach(function (promise) {
                promise._start({_success:null, _error:argument});
            });
        },
        /**
        ...
        promise: function () {
            ...
            if (this.resolved || this.rejected || this.canceled)
                return prom._start({_success:this._success, _error:this._error/*, this._context*/});
            this._promises.push(prom);
            return prom;
        }
    };

    var Promise = promise.Promise = function (options) {
        options = options || {};
        ...
        this._queue = options._queue || [];
        this._oldQueue = options._oldQueue || null;

        this._running = false;
        this._executing = false;
        this._initialised = false;
        this._identity = promise.Promise;
        deep.utils.up(Promise.API, this);
    ...
    promise.Promise.prototype = {
        ...
        _initialised: false,
        _start: function (opt) {
            //console.log("start _______________ ", opt)
            if(opt)
                for(var i in opt)
                    this[i] = opt[i];
            this._initialised = true;
            this._error = (this._success instanceof Error) ? this._success : this._error;
            this._success = (this._success instanceof Error) ? null : this._success;
            this._forceHandle();
            return this;
        },
        _forward:function(clone){
            return {
                _state: (clone?deep.utils.copy(this._state):this._state),
                _context: this._context,
                _queue: ((clone && this._queue)?this._queue.slice():[]),
                _oldQueue: ((clone && this._oldQueue)?this._oldQueue.slice():null),
                _success: this._success,
                _error: this._error,
                _rethrow : this._rethrow
            };
        },
        clone:function(){
        ...
        modes : function(arg, arg2)
        {
            ...
            func._isDone_ = true;
            return utils.addInChain.call(self,func);
        },
        /**

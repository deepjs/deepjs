...
define(["require", "../protocol", "../sheet"], function (require) {
	...
	return function(deep){
		...
		deep.Store.extendsChain = function (handler) {
			handler.range = function (arg1, arg2, query, options) {
				...
				var func = function (s, e) {
					...
					.done(function(storeHandler){
						var store = storeHandler.store, method = store.range;
						if (!method)
				...
				var func = function (s, e)
				{
					...
					.done(function(storeHandler){
						...
						.done(function (success) {
							...
							self2._success = self._success = success;
						});
					});
				};
				func._isDone_ = true;
			...
			handler.post = function (object, options) {
				...
				var func = function (s, e) {
					...
					.done(function(storeHandler){
						...
						var store = storeHandler.store, method = store.post;
						if (!method)
						...
						return deep.when(method.call(store, object || deep.chain.val(self), options))
						.done(function (success) {
							self._nodes = [deep.utils.createRootNode(success)];
						});
					});
			...
			handler.put = function (object, options) {
				...
				var func = function (s, e) {
					return deep.protocol.getStoreHandler(self._store || self._storeName)
					.done(function(storeHandler){
						var store = storeHandler.store, method = store.put;
						if (!method)
			...
			handler.patch = function (object, id, options) {
				...
				var func = function (s, e) {
					return deep.protocol.getStoreHandler(self._store || self._storeName)
					.done(function(storeHandler){
						var store = storeHandler.store, method = store.patch;
						if (!method)
						...
						if(method._deep_ocm_)
							method = method();
						return deep.when(method.call(store, object || deep.chain.val(self), id, options))
						.done(function (success) {
							self._nodes = [deep.utils.createRootNode(success)];
						});
					});
			...
			handler.del = function (id, options) {
				...
				var func = function (s, e) {
					return deep.protocol.getStoreHandler(self._store || self._storeName)
					.done(function(storeHandler){
						var store = storeHandler.store, method = store.del;
						if (!method)
			...
			handler.rpc = function (method, body, uri, options) {
				...
				var func = function (s, e) {
					return deep.protocol.getStoreHandler(self._store || self._storeName)
					.done(function (storeHandler) {
						var store = storeHandler.store, action = store.rpc;
						if (!action)
							return deep.errors.Store("provided store doesn't have RPC. aborting RPC !");
						if(action._deep_ocm_)
							action = action();
						return deep.when(action.call(store, method, body, uri, options))
							.done(function (success) {
							self._nodes = [deep.utils.createRootNode(success)];
						});
					});
			...
			handler.bulk = function (arr, uri, options) {
				...
				var func = function (s, e) {
					return deep.protocol.getStoreHandler(self._store || self._storeName)
					.done(function(storeHandler){
						var store = storeHandler.store, method = store.bulk;
						if (!method)
		...
		deep.store.filterPrivate = function(metod)
		{

...
define(["require", "deepjs/deep", "./store"], function (require, deep, stores)
{

    var filterPrivate = stores.store.filterPrivate = function(method) {
        return function(old){
	...
	var applyPatch = function(item, query, object) {
		if(query)
		{
			var r = deep.query(item, query, { resultType:"full", allowStraightQueries:false });
			r.forEach(function(entry){
				entry.value = deep.utils.up(object, entry.value, entry.ancestor.value, entry.key);
			});
		}
		else
	...
	deep.store.fullSheet = {
		...
		.before(stores.managePathOptions),
		...
		"dq.bottom::./!":{
			...
			checkReadOnly:function(object, data, options, schema){
				...
				if(this._deep_object_)
                    schema = deep.utils.schemaByValuePath(schema, options.id+(options.path||""), '/');
				var nodes = deep.query(data, ".//?_schema.readOnly=true", { resultType:'full', schema:schema });

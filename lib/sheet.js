	...
	return function(deep)
	{
		...
		deep.sheeter = {
			...
			backgrounds:function(){
				...
				var sheeter = getSheeter(this);
				sheeter.queue.push(function(entry){
					deep.utils.bottom({backgrounds:args}, entry.value);
					...
					.done(function(r){
						...
						if(r)
							r.forEach(function(item){
								...
								if(item._deep_query_node_)
									value = item.value;
								var f = deep.utils.up(layer, value);
								if(item.ancestor)
					...
					.done(function(r){
						...
						if(r)
							r.forEach(function(item){
								...
								if(item._deep_query_node_)
									value = item.value;
								var f = deep.utils.bottom(layer, value);
								if(item.ancestor)
									item.ancestor.value[item.key] = f;
								modified.push(f);
							});
					...
					.done(function(r){
						...
						if(r)
							r.forEach(function(item){
								...
								if(item._deep_query_node_)
									value = item.value;
								deep.utils.bottom({backgrounds:(layer.forEach?layer:[layer])}, value);

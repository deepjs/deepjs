...
define(["require", "./utils", "./protocol", "./compiler", "./errors", "./promise", "./logs", "./nodes", "./protocol"], function(require, utils, proto, compiler, errors, promise, logs, nodes, protoc) {
	var sheets = {};
	...
	sheets.sheet = function() {
		...
		if(arguments.length > 2)
		{
			...
			for (var i = arguments.length - 2; i >= 0; i--)
				sheet = compiler.aup(arguments[i], sheet);
			*/
			var args = Array.prototype.slice.call(arguments);
			entry = args[0];
			args[0] = {};
			sheet = deep.up.apply(deep, args);
		}
		else
		{
			//entry = arguments[1];
	...
	nodes.sheet = function(s, shts) {
		if(!s)
			return s;
		return promise.when(protoc.getAll(shts))
		.done(function(objects) {
			objects.unshift({});
			var finalSheet = deep.up.apply(deep, objects);
			if(s._deep_array_)
			{
				var promises = [];
				...
				return sheets.sheet(s, finalSheet).when(s);
		});
	};	

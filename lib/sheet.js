...
define(["require", "./utils", "./protocol", "./compiler"], function (require, utils, proto)
{
	...
	sheet.sheet = function (sheet, entry, options)
	{
		options = options || {};
		options.entry = entry;
		// console.log("SHEET  : start : ", entry.a);
		var res = [];
		var report = {};
		Object.keys(sheet).forEach(function(i){
			...
			.done(function(handler){
				// console.log("SHEET  : before handler : ", entry.a);
				return handler(sheet[i], options);
			})
			.done(function(s){
				// console.log("SHEET APPLICATION RESULT : ", s)
				report[i] = s;
			});
			res.push(d);
		});
	...
	sheet.sheeter = {
		...
		up:function(){
			...
			var sheeter = getSheeter(this);
			sheeter.queue.push(function(entry){
				// console.log("sheeter.up : entry : ", entry, "  ----- args : ", args);
				var d = deep(entry);
	...
	proto.protocols.SheetProtocoles = {
		...
		_backgrounds:function (request, options) {
			options = options || {};
			var self = this;
			return function dodqBottom(layer){
				options.allowStraightQueries = false;
				...
				.done(function(r){
					...
					if(r)
						r.forEach(function(item){
							...
							if(item._deep_query_node_)
								value = item.value;
							utils.bottom({_backgrounds:(layer.forEach?layer:[layer])}, value);
				...
				.done(function(r){
					...
					if(r)
						r.forEach(function(item){
							...
							if(item._deep_query_node_)
								value = item.value;
							var d = deep.wired(sheeter.queue, [value], {})
							.done(function(res){
								item.value = res;
								if(item.ancestor)
		...
		nodes:function (request, options) {
			options = options || {};
			var self = this;
			return function doDqTransform(fn){
				options.allowStraightQueries = false;
				...
				.done(function(rs){
					...
					if(rs)
						rs.forEach(function(item){
							var r = fn(item);
							if(r !== undefined && (r.then || r.promise))
							{
								async = true;
								res.push(r);
							}

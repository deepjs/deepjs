	...
	return function(deep) {
			...
			for (var i = 0; i < treatments.length; ++i) {
				...
				if (treatment.what) {
					...
					if (typeof treatment.what === 'string') {
						...
						treatment.what = deep.get(treatment.what, {
							root: context
						});
		...
		deep.applyTreatments = function(treatments, context, keepSended) // apply render and place in dom orderedly
		{
			...
			treatments.forEach(function(treatment) {
				...
				if (keepSended)
					if (treatment.source)
						treatment.source.sended = function() {
							return sended;
						};
					else
						treatment.sended = function() {
							return sended;
						};
